...
//note that the fomula for things is: Min = k * minLength, Max = k* maxLength
...
public class Main{
    public static void main(String[] b){
        ...
        System.out.println("Please enter the number of Strings inside of A: ");
        int setA = Integer.parseInt(something.nextLine());

        System.out.println("Please now enter a value for k (A^k): ");
        int k = Integer.parseInt(something.nextLine());

       int min = Integer.MAX_VALUE;
       int max = Integer.MIN_VALUE;


        System.out.println("Now enter the actual strings in the A (it doesnt really matter): ");
        for(int innie=0; innie<setA;innie++){
            int lengthA = something.nextLine().length();
            if(lengthA<min){
                min=lengthA;
            }
            if (lengthA>max){
        ...
        System.out.println("A^"+k+", ["+(k*min)+", "+(k*max)+"]");
        something.close();
    }
}